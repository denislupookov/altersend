...
import { restoreFromStore, type PurchaseContext } from './purchaseFlow'
...
function makeContext({
  ...
  const ctx = {
    client: {
      status: async () => ({ exists: storedExists, active: true, validUntil: null }),
  ...
  return { ctx, actions, discarded, code: () => written }
}
